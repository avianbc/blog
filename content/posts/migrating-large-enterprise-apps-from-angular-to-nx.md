...
Migrating to an Nx integrated monorepo offered several immediate wins:
...
- Renaming `tsconfig.json` → `tsconfig.base.json` (Nx convention)
...
Each app's Docker image was tagged with a build number derived from the project-scoped pipeline counter (`$CI_PIPELINE_IID` in GitLab CI). When all pipelines moved to the new monorepo, this counter reset to 1 — and one of the archived repos' counters had been in the thousands. Build `1` in the monorepo would collide with old images from the archived repo in the container registry.
...
When the most complex component library became the monorepo seed, its `package.json` only contained that library's dependencies. Before merging the remaining repos, all their dependencies had to be pre-installed — adding ~56 entries to `package.json` and ~33,000 lines to `package-lock.json` in a single commit.
...
The monorepo has continued evolving since the September 2023 migration:

| Period | Angular | TypeScript | Nx |
|---|---|---|---|
| Migration (Sep 2023) | 16 | ~4.9 | 16 |
| 16 months post-migration | 16 → 18 | 4.9 → 5.5 | 16 → 19 |
| ~18 months post-migration | 16 → 19 (mega-upgrade, 500+ files) | 5.5 | 19 |
| ~2.5 years post-migration | 19 | ~5.7 | ~20.8 |

Several new shared libraries were added post-migration following modern patterns. The `README` explicitly directs new work to the newer libs — the migrated ones carry their migration-era conventions forward and are in maintenance mode.

The Angular 16 → 19 upgrade (~18 months after migration) was the most significant post-migration change: hundreds of files affected, the largest single commit in the repo's history.
...
### Recommended Approach for Future Similar Migrations
